...
export default class componentName extends Component {
    ...
    if(this.state){
        ...
        return <Card key={item.name}  name={item.name} image = {item.image} price={item.price} btntext='ADD To BAG' width='600px' onCardClick={()=>{this.cardClick(item)}}/>
        })
        return cards
    }
    else{
    ...
    renderFavourites = () =>{
        if(this.state){
            ...
            return <Card key={item.name} name={item.name} image = {item.image} price={item.price} btntext='REORDER'width='400px'imgH='300px' imgW='300px'/>
            })
            return cards
        }
        else{
    ...
    rederCategories = () =>{
        if(this.state){
            ...
            return  categories.map((item)=>{
                 return <Categories key={item.name} name = {item.name} image = {item.image} filter={this.filter}/>
             })
        }
    }
